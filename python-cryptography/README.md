...
Recall from our ECB mode discussion and demonstration that producing the same ciphertext for the same plaintext can allow the secret to be completely recovered **even without knowledge of the secret key**. Let's now extend this concept to CBC mode. We know that CBC mode "protects" against this case, but **only if the same IV+secret_key combination is never re-used.** The reason that's critical is because re-using the same IV+secret_key combination effectively "devolves" into the ECB case!
